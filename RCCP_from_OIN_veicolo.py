...
# 15/07/2025
#   RCCP 2026 Veicolo - ok
#   Colori standard - ok
#   Info turni - ok
#   Percentuale totale saturazione - ok
#  Adattamento altri anni
...
st.header('Caricamento CdC Veicolo', divider='red')
uploaded_cdc_veicolo = st.file_uploader(
    "Carica file CdC veicolo 20xx.xlsx)",
)
if not uploaded_cdc_veicolo:
    st.stop()
...
df_famiglia_linea_mese_veicolo = pd.read_excel(uploaded_cdc_veicolo)#, parse_dates=True, index_col=0)
...
df_melt = df_melt[df_melt['CDC'].isin(cdc_veicolo)]
...
uploaded_file = st.file_uploader(
    "Carica PPP 20xx (PR74OP 20xx Bologna_esp xxx_.xlsx)",
    #accept_multiple_files=True # caricarli tutti e poi selezionare 2026 (in un secondo momento)
)
...
pivot_df = pivot_df[pivot_df['CDC'].isin(cdc_veicolo)]
